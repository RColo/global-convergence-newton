...
\author{
  ...
  Fynn Gohlke\\
  University of Basel\\
  \texttt{fynn.tohlke@stud.unibas.ch} \\
  \AND
  Benedikt Heuser\\
  University of Basel\\
...
Finally we will propose two modifications of the algorithms to achieve an decrease in runtime, by either coupling the Newton-type method with a conjugate gradient method for Hessian vector multiplication or Strassen's algorithm for fast matrix inversion.
\section{Background}
...
for symmetric problems, applying CG to newton drops the effort for conversion down to 
$$O(k\cdot n^2) = O(\sqrt{\kappa}\log \left( 1/\epsilon\right) \cdot n^2)$$ where $\kappa(H) = \frac{\lambda_{max}(H)}{\lambda_{min}(H)}$\\
SSOR can precondition H to reduce the condition number.
