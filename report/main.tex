...
Finally we will propose two modifications of the algorithms to achieve a decrease in runtime, by either coupling the Newton-type method with a conjugate gradient method for Hessian vector multiplication or Strassen's algorithm for fast matrix inversion.
\section{Background}
...
In his 2023 article Mishchenko presents a variation of Newton's method that uses the update rule \cite{mishchenko2023regularized}:
...
He the proposes several algorithms, which use the following constants:
\begin{itemize}
    \item \(x_i\in \mathbb{R}^d\), the estimate in the i-th iteration 
    \item \(H>0\), a constant, which has to be chosen s.t. the Hessian is 2H-Lipschitz.
    \item \(I_d\), the Identity matrix.
    \item \(\lambda_i\), regularizer of i-th iteration
    \item \(H_i\), an estimate of H.
    \item \(n_k\),iteration counter
    \item \(M_k<H_k, M_k<H\), a local estimate on the smoothness of the Hessian
\end{itemize}
\subsubsection{Globally convergent regularized Newton method for minimization}
given \(x_0,H\),\\
for\(\quad k\in \mathbb{N}_0\{\)\\
\(\lambda_k=\sqrt{H||\nabla f(x_k)||}\)\\
\(x_{k+1}=x_k-(\nabla^2 f(x_k)+\lambda_k I_d)^{-1}\nabla f(x_k); \)\qquad Compute \(x_{k+1}\) by solving a linear system.\\
\(\}\)

\subsubsection{Adaptive Newton AdaN}

given \(x_0,H_0>0\),\\
for\(\quad k\in \mathbb{N}_0\{\)\\
Initialize line search with reduced regularization \(H_k=\frac{H_{k-1}}{4}\)\quad for\quad \(k>0\)\(,n_k=0\)\\
do\(\{\)\\
\(H_k*=2;\)\quad Increase Regularization\\
\(n_k++\)\\
\(\lambda_k=\sqrt{H_k||\nabla f(x_k)||}\)\\
\(x_+=x_k-(\nabla^2 f(x_k)+\lambda_kI)^{-1}\nabla f(x_k); \)\quad New trial point\\
\(r_+=||x_+-x_k||\)\\
\(\}\)\quad while \(||\nabla f(x_+)||>2\lambda_kr_+\)\quad or \(f(x_+)>f(x_k)-\frac{2\lambda_kr_+^2}{3}\)\\
\(x_{k+1}=x_+;\)\quad Select trial point\\
\(\}\)

\subsubsection{Heuristic modification of AdaN}
given \(x_0 \neq x_1\)
Initialize \(H_0=\frac{||\nabla f(x_1)-\nabla f(x_0)-\nabla^2f(x_0)(x_1-x0)||}{||x_1-x_0||^2}\)\\
for \(k\in \mathbb{N}^+\)\{\\
\(M_k=\frac{||\nabla f(x_k)-\nabla f(x_{k-1})-\nabla^2f(x_{k-1})(x_k-x_{k-1})||}{||x_k-x_{k-1}||^2}\)\\
\(H_k=max\{M_k,\frac{H_{k-1}}{2}\}\)\\
\(\lambda_k=\sqrt{H_k||\nabla f(x_k)||}\)\\
\(x_{k+1}=x_k-(\nabla^2 f(x_k)+\lambda_k I_d)^{-1}\nabla f(x_k)\)
\(\}\)
...
In symmetric cases, applying CG to newton drops the effort for conversion down to 
$$O(k\cdot n^2) = O(\sqrt{\kappa}\log \left( 1/\epsilon\right) \cdot n^2)$$ where $\kappa(H) = \frac{\lambda_{max}(H)}{\lambda_{min}(H)}$\\
Preconditioning with SSOR can reduce condition number.
